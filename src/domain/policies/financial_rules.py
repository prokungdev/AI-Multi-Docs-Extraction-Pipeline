"""Financial rules and domain validation policies.
...
class FinancialMathValidator(BaseValidator):
    ...
    def validate(self, payload: dict, context: dict = None) -> tuple[dict, bool, list[str]]:
        ...
        if isinstance(totals, dict):
            ...
            # Check: Net = Subtotal - Discount + VAT
            calc_net = subtotal - discount + vat_amount
            tolerance = 0.05

            if abs(calc_net - net_amount) > tolerance and net_amount > 0:
                needs_review = True
                reasons.append(
                    f"Financial Math Inconsistency: Calculated net ({calc_net:.2f}) != Extracted net ({net_amount:.2f})"
                )
...
class ValidationStrategyEngine:
    """
    Orchestrates execution of registered validation strategies against a document payload.
    """
    ...
    def register(self, validator: BaseValidator) -> None:
        """Registers a custom validation strategy into the execution pipeline."""
        self.validators.append(validator)

    def run_validation(self, payload: dict, context: dict = None) -> tuple[dict, bool, list[str]]:
        """
        Runs all registered validation strategies sequentially on the payload.
        """
        combined_needs_review = False
        all_reasons = []

        current_payload = payload
        for v in self.validators:
            current_payload, needs_review, reasons = v.validate(current_payload, context)
            if needs_review:
                combined_needs_review = True
            all_reasons.extend(reasons)

        return current_payload, combined_needs_review, all_reasons
