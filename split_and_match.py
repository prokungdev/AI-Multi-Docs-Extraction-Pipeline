...
def main():
    setup_logger()
    logger.info("==========================================")
    logger.info("  Run_02_Split_and_Match: Splitting and Matching")
    logger.info("==========================================")
    ...
    if not os.path.exists(inbox_dir):
        logger.error(f"Inbox directory does not exist: {inbox_dir}")
        return
        
    logger.info(f"Scanning inbox for files to process in: {inbox_dir}")
    processed_count = 0
    total_pages_count = 0
        ...
        for file in files:
            if file.lower().endswith((".pdf", ".png", ".jpg", ".jpeg")):
                ...
                try:
                    ...
                    if is_dup:
                        logger.warning(f"Duplicate file detected (hash: {file_hash[:12]}...). Skipping. Already in batch: {dup_meta['batch_id']}")
                        continue
                except Exception as he:
                    logger.error(f"Error checking duplicate for {filename}: {he}")
                    continue
                ...
                if os.path.dirname(parent_dir) == inbox_abs:
                    ...
                    logger.info(f"Matched source (from folder structure): '{source}'")
                else:
                    # File is at the root of inbox, run matcher
                    ...
                    if file_lower.endswith(".pdf"):
                        try:
                            ...
                            logger.error(f"Failed to split temp page for matching: {e}")
                            continue
                    else:
                    ...
                    if temp_paths:
                        ...
                        logger.info(f"Matched source (from AI matcher): '{source}'")
                        shutil.rmtree(temp_split_dir, ignore_errors=True)
                    else:
                        logger.warning("Failed to generate temp page for matching. Falling back to '_default'")
                        source = "_default"
                
                # Check active status of source
                if not is_source_active(domain, source):
                    logger.warning(f"Source '{source}' is inactive in settings. Falling back to '_default'")
                    source = "_default"
                ...
                if file_path != dest_raw_path:
                    ...
                    logger.info(f"Moved raw file to: {dest_raw_path}")
                ...
                if file_lower.endswith(".pdf"):
                    try:
                        ...
                        logger.error(f"Failed to split PDF: {se}")
                        continue
                else:
                ...
                # Rename the files systematically and write to database directly as PREPROCESSED
                ...
                for idx, temp_path in enumerate(raw_split_paths):
                    ...
                    logger.info(f"Registered Page {page_num}/{len(raw_split_paths)}: {new_filename}")
                    total_pages_count += 1
                
                processed_count += 1
                
    logger.info("==========================================")
    logger.info("  Step 2: Split and Match Summary")
    logger.info("==========================================")
    logger.info(f"Successfully processed raw files: {processed_count}")
    logger.info(f"Successfully split and registered pages: {total_pages_count}")
    logger.info("==========================================")
